...
interface FloralInventoryGridProps {
...
export default function FloralInventoryGrid({ inventoryItems }: FloralInventoryGridProps) {
    ...
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-8">
        {inventoryItems.map((item) => {
          ...
          const itemImage = IMAGE_MAP[item.name] || "https://images.unsplash.com/photo-1563241527-3004b7be0ffd?w=120&h=120&fit=crop"
        ...
        <DialogContent className="sm:max-w-md border border-border/85 shadow-2xl rounded-2xl bg-card/95 backdrop-blur-md">
          <DialogHeader className="flex flex-row items-center gap-4 text-left border-b border-border/40 pb-4">
            <div className="relative w-16 h-16 rounded-xl overflow-hidden border border-border/60 bg-muted shrink-0 shadow-xs">
              {selectedItem && (
                <Image
                  src={IMAGE_MAP[selectedItem.name] || "https://images.unsplash.com/photo-1563241527-3004b7be0ffd?w=120&h=120&fit=crop"}
